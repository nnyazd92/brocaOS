...
3. **Token Generated**: UUID token with expiration timestamp (default: 5 minutes)
...
- **Request-bound**: Each token is tied to a specific approval request
- **Non-transferable**: Tokens are validated against the approval system instance that created them

### Token Expiration

Tokens expire at their `expires_at` timestamp. Once expired:
- Verification returns `valid=False` with error "Token expired"
- Token cannot be used for new operations
- A new approval token must be generated from the approved request

**Note**: If you need to extend token lifetime, generate a new token from the same approved request before the current token expires.
...
**Cause**: Token doesn't exist in the approval system (invalid token, wrong system instance, or token was never generated).

**Resolution**:
1. Verify you're using the correct token from `approve_actuator_request`
2. Ensure the token was generated by the same `ApprovalSystem` instance
...
{
    "success": false,
    "error": "Invalid approval token: Approval token not found. The token may be invalid or may have been generated by a different approval system instance."
}
```
...
**Cause**: Token's `expires_at` timestamp has passed (default: 5 minutes after generation).

**Resolution**:
1. Generate a new approval token from the same approved request
2. Use the new token for the operation
3. For longer-lived tokens, generate new tokens periodically

**Example Error**:
```json
{
    "success": false,
    "error": "Invalid approval token: Approval token expired at 2024-01-01T12:05:00Z. Request a new approval token."
}
```
...
If you create tokens in one `EnvironmentAccessSystem` instance and try to use them in another, verification will fail because tokens are stored in instance-specific dictionaries. Ensure you're using the same system instance throughout the workflow.
